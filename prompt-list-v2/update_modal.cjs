...
// 2. Update Modal Reset
code = code.replace(
  /setIsPayoutModalOpen\(false\);\r?\n\s*setPayoutDetails\(''\);/g,
  `setIsPayoutModalOpen(false);\n        setPayoutDetails('');\n        setPayoutAgreed(false);`
);

// 3. Update the Modal JSX
const oldModalJsx = /\{isPayoutModalOpen && \([\s\S]*?<\/button>\r?\n\s*<\/div>\r?\n\s*<\/div>\r?\n\s*<\/div>\r?\n\s*\)\}/;

const newModalJsx = `{isPayoutModalOpen && (
...
console.log("Updated CreatorDashboardPage.tsx payout modal");
