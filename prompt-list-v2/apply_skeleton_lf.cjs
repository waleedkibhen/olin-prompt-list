const fs = require('fs');
let rawCode = fs.readFileSync('src/components/PromptCard.tsx', 'utf8');
...
if (code.includes(effTarget)) {
  ...
  console.log('Replaced effectiveMonetization target');
}
...
if (code.includes(effectTarget)) {
  ...
  console.log('Replaced effect target');
}
...
if (code.includes(promptJsxTarget)) {
  ...
  console.log('SUCCESS: Replaced prompt JSX with skeleton loader!');
} else {
  console.log('FAILED to match promptJsxTarget directly after LF normalization');
}
