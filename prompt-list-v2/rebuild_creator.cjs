...
code = code.replace(
  '<button className="btn-solid" style={{ padding: \'0.5rem 1rem\', fontSize: \'0.85rem\', display: \'flex\', alignItems: \'center\', gap: \'0.4rem\' }}>\n                  <DollarSign size={14} /> Request Payout\n                </button>',
  `<button 
                  className="btn-solid" 
...
const adRevenueRegex = /\{monetizationStats\.isAdRevenuePending && \([\s\S]*?\{showMonetizationInfo && \([\s\S]*?<\/button>\s*<\/div>\s*\)\}/;
if (adRevenueRegex.test(code)) {
  code = code.replace(
    adRevenueRegex,
...
const paidTagRegex = /<span className=\{styles\.badgePill\} style=\{\{ backgroundColor: 'rgba\(59, 130, 246, 0\.1\)', color: '#3b82f6', border: '1px solid rgba\(59, 130, 246, 0\.2\)', display: 'inline-flex', alignItems: 'center', gap: '0\.25rem' \}\}>\s*<Lock size=\{12\} \/> Paid \(\$\{post\.price\?\.toFixed\(2\) \|\| '1\.99'\}\)\s*<\/span>/g;
code = code.replace(paidTagRegex, `<span style={{ color: '#8b5cf6', display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.9rem', fontWeight: 500 }}><Lock size={14} /> Paid Prompt</span>`);

const adTagRegex = /<span className=\{styles\.badgePill\} style=\{\{ backgroundColor: 'rgba\(139, 92, 246, 0\.1\)', color: '#8b5cf6', border: '1px solid rgba\(139, 92, 246, 0\.2\)', display: 'inline-flex', alignItems: 'center', gap: '0\.25rem' \}\}>\s*<PlayCircle size=\{12\} \/> Ad Unlock\s*<\/span>/g;
code = code.replace(adTagRegex, `<span style={{ color: '#10b981', display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.9rem', fontWeight: 500 }}><PlayCircle size={14} /> Ad Unlock</span>`);
...
console.log('Creator dashboard rebuilt!');
