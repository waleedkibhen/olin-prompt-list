...
// 1. ADD STATES
if (!code.includes('isPayoutModalOpen')) {
  code = code.replace(
...
// 4. ADD HANDLE SUBMIT
if (!code.includes('handleRequestPayout')) {
  code = code.replace(
...
const infoBoxRegex = /\{showMonetizationInfo && \(\s*<div style=\{\{ backgroundColor: 'rgba\(59, 130, 246, 0\.05\)', border: '1px solid rgba\(59, 130, 246, 0\.2\)', borderRadius: '12px', padding: '1\.25rem', marginBottom: '2rem', display: 'flex', gap: '1rem', position: 'relative' \}\}>\s*<div style=\{\{ color: '#3b82f6', flexShrink: 0, marginTop: '0\.25rem' \}\}>\s*<Info size=\{24\} \/>\s*<\/div>\s*<div>\s*<h3 style=\{\{ margin: '0 0 0\.5rem 0', fontSize: '1\.1rem', fontWeight: 600 \}\}>How Creator Monetization Works<\/h3>\s*<ul style=\{\{ margin: 0, paddingLeft: '1\.25rem', color: 'var\(--text-secondary\)', display: 'flex', flexDirection: 'column', gap: '0\.5rem', fontSize: '0\.95rem' \}\}>\s*<li><strong>Paid Prompts:<\/strong> You earn directly when users buy your prompt via Whop \(minus platform fees\)\.<\/li>\s*<li><strong>Ad-Supported Prompts:<\/strong> Prompts stay 100% free for viewers\. Revenue is pooled and calculated at the end of each month based on your share of global page views\.<\/li>\s*<li><strong>Payouts:<\/strong> Minimum withdrawal threshold is <strong>\$10\.00<\/strong>\.<\/li>\s*<\/ul>\s*<\/div>\s*<button \s*onClick=\{\(\) => setShowMonetizationInfo\(false\)\}\s*style=\{\{ position: 'absolute', top: '1rem', right: '1rem', background: 'transparent', border: 'none', color: 'var\(--text-muted\)', cursor: 'pointer', padding: '0\.25rem' \}\}\s*title="Dismiss"\s*>\s*<X size=\{18\} \/>\s*<\/button>\s*<\/div>\s*\)\}/;

if (infoBoxRegex.test(code)) {
  code = code.replace(
    infoBoxRegex,
...
console.log('Successfully rewrote CreatorDashboardPage.tsx');
