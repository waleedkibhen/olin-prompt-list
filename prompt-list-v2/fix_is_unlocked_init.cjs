...
const regex = /const \[isUnlocked, setIsUnlocked\] = useState<boolean>\(false\);/;

const replacement = `const [isUnlocked, setIsUnlocked] = useState<boolean>(() => {
    // Synchronously determine initial unlock state to prevent a 1-frame "locked" flash for free posts
