...
const sectionMatch = code.match(/<section className=\{styles\.kpiGrid\}>[\s\S]*?(?=<\/main>)/);

if (sectionMatch) {
    const performanceContent = sectionMatch[0];
    const newContent = `{activeTab === 'performance' ? (
            <>
              ${performanceContent.replace(/<\/main>$/, '')}
            </>
          ) : (
            <>
    ...
    code = code.replace(sectionMatch[0], newContent + '\n      </main>');
    fs.writeFileSync('src/pages/CreatorDashboardPage.tsx', code);
    console.log('Replaced performance content with tabbed logic');
} else {
    console.log('Could not find section Match');
}
