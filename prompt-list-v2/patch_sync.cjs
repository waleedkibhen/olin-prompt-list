...
const replaceEffect = `    let serverUnlocked = false;
    ...
    if (user && userProfile && userProfile.purchasedPrompts && userProfile.purchasedPrompts.includes(post.id)) {
      serverUnlocked = true;
...
console.log('fixed promptcard sync issues again');
