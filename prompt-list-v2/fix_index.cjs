const fs = require('fs');
let code = fs.readFileSync('src/pages/CreatorDashboard/index.tsx', 'utf8');

const returnRegex = /return\s*\(\s*<main className=\{styles\.container\}>[\s\S]*?\);\s*\}$/;
...
if (returnRegex.test(code)) {
  ...
  console.log("Regex did not match. Let's find index of 'return ('");
  let idx = code.lastIndexOf('return (');
  if (idx !== -1) {
