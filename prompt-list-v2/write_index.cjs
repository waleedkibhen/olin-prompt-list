...
code = code.replace("import React", newImports + "\nimport React");

// Remove the inline TrendIndicator
code = code.replace(/const TrendIndicator = \(\{ trend \}: \{ trend: number \| null \}\) => \{[\s\S]*?^  \};/m, '');
...
code = code.replace(/return \(\s*<main className=\{styles\.container\}>[\s\S]*?\);\s*\}$/, newReturn);

fs.writeFileSync('src/pages/CreatorDashboard/index.tsx', code);
console.log("Written index.tsx");
